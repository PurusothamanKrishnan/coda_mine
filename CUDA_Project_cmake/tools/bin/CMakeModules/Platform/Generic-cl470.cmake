# - Generic compiler support for TI TMS470 (ARM) family
...
# This module defines:
#
#  TI_470		 	 - Set to True if this file is included.
#
...
if(NOT TI_470)
	...
	#
	find_program(AR_EXE "ar470" PATHS ${CMAKE_FIND_ROOT_PATH} NO_DEFAULT_PATH)
	set(CMAKE_AR "${AR_EXE}" CACHE FILEPATH "TI archiver" FORCE)

	find_program(LNK_EXE "lnk470" PATHS ${CMAKE_FIND_ROOT_PATH} NO_DEFAULT_PATH)
	set(LINKER "${LNK_EXE}" CACHE FILEPATH "TI linker" FORCE)

	#
	# Find include path
	#
	find_path(_INCLUDE_PATH "Platform_Types.h" PATHS ${CMAKE_FIND_ROOT_PATH} NO_DEFAULT_PATH)
	include_directories(${_INCLUDE_PATH})

	#
	# Find library path
	#
	find_path(_LIB_PATH "rtsv7A8_T_le_n_v3_eabi.lib" PATHS ${CMAKE_FIND_ROOT_PATH} NO_DEFAULT_PATH)
	...
	set(CMAKE_ASM_TMS470_OUTPUT_EXTENSION_REPLACE 1)
	...
	#
	# Ensure that the library suffix is not stripped when CMake "-l" processing takes place.
	#
	set(CMAKE_LINK_LIBRARY_SUFFIX  		${CMAKE_STATIC_LIBRARY_SUFFIX})

	#
	# Default compiler flags (this can only be overridden in the Toolchain file, not in subsequent CMakeLists.txt)
	#
	set(CMAKE_C_FLAGS_INIT					"${CMAKE_C_FLAGS_INIT} -c")
...
#
# TI_START_LINK_FLAGS and TI_END_LINK_FLAGS will only be set on second CMake pass, so ignore TI_470 and always do this.
#
